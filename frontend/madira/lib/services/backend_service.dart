...
// ===================================================================
// lib/services/backend_service.dart - CROSS-PLATFORM WORKING VERSION
// ===================================================================
import 'package:flutter/material.dart';
import 'package:file_picker/file_picker.dart';
import 'package:shared_preferences/shared_preferences.dart';
  ...
  Future<bool> startBackend() async {
    ...
    try {
      ...
      if (responding) {
        ...
        _errorMessage =
            'Backend not responding. Check if Python packages are installed.';
        _needsConfiguration = true;
        await _killAllPythonProcesses();
        notifyListeners();
        return false;
  ...
  void _monitorOutput() {
    ...
    _process?.stderr
        ...
        .listen((line) {
          ...
          // Recognize INFO logs as non-errors
          if (line.contains('INFO:') ||
              line.contains('Uvicorn running') ||
              line.contains('Started server process') ||
              line.contains('Application startup complete') ||
            ...
            if (line.contains('ModuleNotFoundError')) {
              _errorMessage =
                  'Missing Python packages. Run: pip3 install -r requirements.txt';
            }
          }
        });
  ...
  Future<void> _killAllPythonProcesses() async {
    try {
      ...
      if (Platform.isWindows) {
        ...
        // macOS: Use lsof to find processes on port 8000
        try {
    ...
    try {
      ...
      try {
        if (Platform.isWindows) {
          ...
          final lsofResult = await Process.run('lsof', ['-ti', ':8000']);
          if (lsofResult.exitCode == 0) {
            final pids = lsofResult.stdout.toString().trim().split('\n');
            for (final pid in pids) {
              if (pid.isNotEmpty) {
